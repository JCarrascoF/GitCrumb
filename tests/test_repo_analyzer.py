...
class TestFindGitRoot(unittest.TestCase):
    ...
    @patch("gitcrumb.repo_analyzer.subprocess.run")
    def test_returns_root_when_git_found(self, mock_run):
        ...
        from gitcrumb.repo_analyzer import _find_git_root
...
class TestIsIgnoredByParent(unittest.TestCase):
    ...
    @patch("gitcrumb.repo_analyzer._find_git_root")
    def test_ignored_path(self, mock_find_root):
        mock_find_root.return_value = Path("/home/user/project")

        with patch("gitcrumb.repo_analyzer.subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(returncode=0)  # ignored
            from gitcrumb.repo_analyzer import _is_ignored_by_parent
...
class TestFindRepos(unittest.TestCase):
    ...
    @patch("gitcrumb.repo_analyzer._is_ignored_by_parent")
    def test_finds_repos(self, mock_ignored):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            from gitcrumb.repo_analyzer import find_repos
...
class TestAnalyzeRepositories(unittest.TestCase):
    ...
    @patch("gitcrumb.repo_analyzer.find_repos")
    @patch("gitcrumb.repo_analyzer.extract_commits")
    @patch("gitcrumb.repo_analyzer.extract_stats")
